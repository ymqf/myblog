...
class Database{
     ...
     * @return mixed|null
     */
    public static function getDataByField($filed,$filed_type,$table){
        $db = Typecho_Db::get();
        try{
            if ($filed_type && !self::createFiledInTable($filed,$filed_type,$table)) {//没有数据，此时会创建该字段
                return null;
            } else {
                // 获取第三个参数后面的可变参数列表,兼容php5.4
                $args = func_get_args();
                $args = array_splice($args,3);
                $row = $db->fetchRow(call_user_func_array(array($db->select($filed)->from('table.'.$table), 'where'), $args));
...
//            var_dump($exception);
    ...
    // 修改某条记录的某个字段
    public static function updateField($filed,$field_value,$table){
        $db = Typecho_Db::get();

        // 获取第三个参数后面的可变参数列表,兼容php5.4
        $args = func_get_args();
        $args = array_splice($args,3);
        $db->query(call_user_func_array(array($db->update('table.'.$table)->rows(array($filed => $field_value)), 'where'), $args));
    }
