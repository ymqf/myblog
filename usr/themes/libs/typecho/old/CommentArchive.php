...
class Handsome_Widget_Comments_Archive extends Widget_Abstract_Comments
{
    ...
    public function data(){
        $this->type="comment";
        $this->cid = $this->parameter->parentId;
        $isLogin = Typecho_Widget::widget('Widget_User')->hasLogin();
        $ret = [];
        $this->options->commentsPageDisplay = "first";//主题侧我们固定把较新的显示在前面了！！

        if ('last' == $this->options->commentsPageDisplay) {
            $currentPage = ceil($this->total / $this->options->commentsPageSize)+1;
        } else {
            $currentPage = 0;
        }

        $parent_content = parent::___parentContent();
        if ($parent_content==null){
            return "[]";
        }
        $path_info = parent::___parentContent()['pathinfo'];
        foreach ($this->_origin_stack as $key=>$item){

            //计算分页
            if ($key % $this->options->commentsPageSize == 0){
                if ('last' == $this->options->commentsPageDisplay) {
                    $currentPage --;
                } else {
                    $currentPage ++;
            ...
            if (strpos($item["text"], '[secret]') !== false) {//提高效率，避免每篇文章都要解析
                if (!$isLogin){
                    continue;
                }
//                $pattern = CommonContent::get_shortcode_regex(array('secret'));
//                $item["text"] = preg_replace_callback("/$pattern/", function ($matches) {
//                    return Content::secretContentParseCallback($matches, true);
//                }, $item["text"]);

            }

            $theId= $this->type . '-' . $item["coid"];;

            if ($this->options->commentsPageBreak) {
                //计算当前属于那一页

                $pageRow = ['permalink' => $path_info, 'commentPage' => $currentPage];
                $permalink = Typecho_Router::url('comment_page', $pageRow, $this->options->index) . '#' . $theId;
            }else{

                $permalink =  $parent_content['permalink'] . '#' . $theId;
    ...
    {
        $sql = $this->db->select('table.comments.coid', 'table.comments.cid', 'table.comments.author', 'table.comments.mail', 'table.comments.url', 'table.comments.ip',
        ...
        if ($this->parameter->tid!=null){
            $sql = $sql->join(
                "table.handsomerelationships",
                "table.handsomerelationships.coid = table.comments.coid",
                'left')
                ->where('table.handsomerelationships.tid=?', $this->parameter->tid);        }
        return $sql;
    }
    ...
    public function execute()
    {
        ...
        $this->_origin_stack = array_values($this->stack);
